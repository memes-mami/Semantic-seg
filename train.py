...
if __name__ == "__main__":
    ...
    ckpt_cb = ModelCheckpoint(
        dirpath=cfg.exp_name,
        filename='{epoch:d}',  # Saves checkpoints as "1.ckpt", "2.ckpt", etc.
        monitor=cfg.monitor,
        mode='max',
        save_top_k=cfg.save_top_k
    )

    # Save the last epoch's checkpoint (overwrites "last.ckpt" every time)
    ckpt_last = ModelCheckpoint(
        dirpath=cfg.exp_name,
        filename='last',  # Always saves as "last.ckpt"
        save_top_k=1,
        save_last=True  # Ensures the last epoch is always saved
    )

    pbar = TQDMProgressBar(refresh_rate=1)

    # Add both best model and last epoch checkpoints
    callbacks = [ckpt_cb, ckpt_last, pbar, lr_monitor]
