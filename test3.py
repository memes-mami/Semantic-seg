...
if __name__ == "__main__":
    ...
    for idx, (image, mask, img_id) in enumerate(test_loader):
        if idx >= 5:  # Stop after processing 5 images
            break

        with torch.no_grad():
            raw_predictions = model(image.cuda(), True)
            ...
            mask_save(results[0])
            print(f"Saved mask {mask_name} ({idx+1}/5)")

    print("Testing complete for first 5 images.")
